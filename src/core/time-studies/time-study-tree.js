...
export class TimeStudyTree {
  ...
  hasRequirements(study, checkOnlyStructure = false) {
    ...
    if (study instanceof ECTimeStudyState) {
      ...
      const hasForbiddenStudies = !Perk.studyECRequirement.isBought && !player.records.fullGameCompletions >= 1 &&
      ...
      const secondaryGoal = Perk.studyECRequirement.isBought || player.records.fullGameCompletions >= 1 || study.isEntryGoalMet;
